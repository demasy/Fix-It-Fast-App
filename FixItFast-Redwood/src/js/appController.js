...
'use strict';
define(['ojs/ojcore', 'knockout', 'dataService', 'appUtils', 'knockout-mapping', 'PushClient', 'OfflineController',
        'ConnectionDrawer', 'ModuleHelper', 'ImageHelper', 'oj-sample-mobile-internal/common/auth-error-translations',
        'ojs/ojconfig','oj-sample-mobile-internal/local-authentication/loader',
...
function (oj, ko, data, appUtils, mapping, PushClient, OfflineController, ConnectionDrawer, moduleHelper, imageHelper, authErrTrsl, Config) {
  ...
  function AppControllerViewModel() {
    ...
    // Get device config
    self.isPhone =  ko.observable();
  
    self.setVhUnits = function() {
      var vh = window.innerHeight * 0.01;
      // Then we set the value in the --vh custom property to the root of the document
      document.documentElement.style.setProperty('--vh', `${vh}px`);
    }

    self.updateHeightAndConfigOnResize = function(){
      self.setVhUnits();
      self.isPhone(Config.getDeviceType() === "phone")
    }

    // set vh and device config on load
    self.updateHeightAndConfigOnResize();

    var resizeTimer;
    $(window).resize(function() {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(self.updateHeightAndConfigOnResize, 100);
    });
    ...
    oj.Router.transitionedToState.add(function(result) {
      ...
      if (newId === 'customerDetails' && oldId === 'incident') {
        self.incidentAnimation('pushEnd');
        return;
      }

      if ((newId === 'incident' || newId === 'createIncident') && oldId === 'incidents') {
        self.incidentAnimation('pushStart');
        self.incidentsAnimation('pushEnd');
        return;
      }

      if ((newId === 'customerDetails' || newId === 'customerCreate') && oldId === 'customers') {
        self.customersAnimation('pushStart');
        return;
      }

      if (newId === 'customers' && (oldId === 'customerCreate' || oldId === 'customerDetails') ){
        self.customersAnimation('pushEnd');
        return;
      }
    ...
    self.incidentAnimation = ko.observable('pushEnd');
    ...
    var animationOptions = {
      ...
      'customerDetails': 'pushStart',
      'customerCreate': 'pushStart',
      'profile': null,
      'about': null,
      'incident': self.incidentAnimation,
      'settings': null,
      'createIncident': 'pushStart'
    };
